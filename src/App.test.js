...
export const handlers = [
  rest.get("/users", (req, res, ctx) => {
    ...
    return res(ctx.delay(150), ctx.json(mockResponseBody));
  }),
...
describe("Main Component", () => {
  ...
  test("It should display all the monsters list in the screen correctly", async () => {
    ...
    await waitFor(() =>
      expect(store.getState().monsters.data.length).toBeGreaterThan(0)
    );
...
describe("Integration Testing", () => {
  test("It should filter the monsters by text entered in search input field", async () => {
    ...
    // by our `Lean` query
    const searchResultsCount = store
      .getState()
      .monsters.data.filter((monster) =>
        monster.name.toLowerCase().includes("lean")
      ).length;

    // asserting that the card-container is euqal to the amount of matches
    expect(container.getElementsByClassName("card-container").length).toBe(
      searchResultsCount
    );
  });
  ...
  test("Our search query matches more than one item", async () => {
    ...
    const searchResultsCount = store
      .getState()
      .monsters.data.filter((monster) =>
        monster.name.toLowerCase().includes("le")
      ).length;
  ...
  test(`Our search query doesn't match any items`, async () => {
    ...
    const searchResultsCount = store
      .getState()
      .monsters.data.filter((monster) =>
        monster.name.toLowerCase().includes("something")
      ).length;
